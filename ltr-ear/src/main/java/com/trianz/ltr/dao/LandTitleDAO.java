...
/**
 * LandTitleDAO - Cloud-ready JDBC Data Access Object for LAND_TITLE table.
 *
 * CLOUD-READY FEATURES:
 *   - Uses HikariCP connection pooling (via CloudDataSourceUtil)
 *   - Async operations with CompletableFuture for high throughput
 *   - Uses java.time.Instant for UTC timestamps
...
public class LandTitleDAO {
    ...
    // Thread pool for async operations (cloud-optimized)
    private static final ExecutorService executorService = Executors.newFixedThreadPool(
        Integer.parseInt(System.getenv().getOrDefault("DAO_THREAD_POOL_SIZE", "10"))
    );
    ...
    // ── Async Insert (Cloud-optimized for high throughput) ────────────────────

    public CompletableFuture<Void> insertAsync(LandTitle t) {
    ...
    // ── Async Find (Cloud-optimized) ───────────────────────────────────────────

    public CompletableFuture<LandTitle> findByTitleNumberAsync(String titleNumber) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                return findByTitleNumber(titleNumber);
            } catch (SQLException e) {
    ...
    // ── Row Mapper (Cloud-ready with Instant) ──────────────────────────────────
    ...
    // ── Shutdown (for graceful application shutdown) ───────────────────────────

    public static void shutdown() {
        executorService.shutdown();
        LOGGER.info("LandTitleDAO executor service shut down");
    }
}
