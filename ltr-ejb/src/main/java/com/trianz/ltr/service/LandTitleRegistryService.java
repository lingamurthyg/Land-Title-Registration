...
/**
 * LandTitleRegistryService - Cloud-native service implementing Land Title Registry business logic.
 * 
 * Cloud-native improvements:
 * - Replaced @Stateless EJB with @Service (Spring)
 * - Replaced EJB CMT with Spring @Transactional
 * - Replaced SessionContext with Spring SecurityContext
 * - Uses java.time.Instant for all timestamps (UTC)
 * - Structured logging with SLF4J
 * - Stateless design for horizontal scaling in cloud
 * - Compatible with AWS ECS, EKS, and containerized deployments
 */
@Service
@Transactional(readOnly = false)
public class LandTitleRegistryService {

    private static final Logger LOGGER = LoggerFactory.getLogger(LandTitleRegistryService.class);
    ...
    public Long initiateTransfer(TitleTransfer transfer) throws LandTitleException {
        try {
            ...
            if (existing.getStatus() != TitleStatus.ACTIVE) {
                throw new LandTitleException(
    ...
    private TitleTransfer findTransferById(Long transferId) throws LandTitleException {
        try {
            ...
            for (TitleTransfer tr : pending) {
                if (transferId.equals(tr.getTransferId())) {
                    return tr;
                }
            }
            throw new LandTitleException(
                    LandTitleException.ErrorCode.TRANSFER_NOT_FOUND,
    ...
    private String getCurrentUsername() {
        try {
            ...
            LOGGER.warn("Failed to get authenticated user, using 'system'", e);
            return "system";
        }
    }
